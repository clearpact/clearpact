...
/// @dev Phase 2 of the ERC-8183 ABI compliance refactor. Inherits all four OZ upgradeable
    ...
    IClearPactExtensions
{
    ...
    /// @dev Resolve the token for a job: per-job override or (future) default token.
    ///      Reverts if no token is configured — caller must setBudget with a token first.
